...
pyuvsim_output_file = "OVRO-LWA_100ms_sim_results.uvfits"
...
uv_center_time.write_uvfits(f"{path}/OVRO-LWA_100ms_sim_center_time.uvfits")

uv_unphased = uv.copy()
uv_unphased.downsample_in_time(n_times_to_avg=uv.Ntimes, allow_drift=True)
uv_unphased.phase_to_time(center_time)
uv_unphased.write_uvfits(f"{path}/OVRO-LWA_100ms_sim_unphased.uvfits")

uv_phased = uv.copy()
uv_phased.phase_to_time(center_time)
uv_phased.downsample_in_time(n_times_to_avg=uv.Ntimes)
uv_phased.write_uvfits(f"{path}/OVRO-LWA_100ms_sim_phased.uvfits")
