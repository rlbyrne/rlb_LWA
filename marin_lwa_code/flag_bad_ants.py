...
# Script adapted from Marin Anderson, Dec 2022
...
def flag_bad_ants(msfile: str) -> str:
    ...
    Nants = len(tables.table(f'{msfile}/ANTENNA', ack=False).getcol('NAME'))
    spws  = tables.table(f'{msfile}/SPECTRAL_WINDOW', ack=False).getcol('NUM_CHAN')
    freqs = tables.table(f'{msfile}/SPECTRAL_WINDOW', ack=False).getcol('CHAN_FREQ').ravel()
    Nspws = len(spws)
    Nchans_perspw = spws[0]
    datacolxx = np.zeros((Nants,Nchans_perspw*Nspws))
    datacolyy = np.copy(datacolxx)
    for antind,tauto in enumerate(tautos.iter("ANTENNA1")):
        for bandind,tband in enumerate(tauto):
            datacolxx[antind,bandind*Nchans_perspw:(bandind+1)*Nchans_perspw] = tband["DATA"][:,0]
            datacolyy[antind,bandind*Nchans_perspw:(bandind+1)*Nchans_perspw] = tband["DATA"][:,3]
    ...
    datacolxxampdb_mask0 = ma.masked_array(datacolxxampdb, mask=flagmask0, fill_value=np.nan)
    datacolyyampdb_mask0 = ma.masked_array(datacolyyampdb, mask=flagmask0, fill_value=np.nan)
    medamp_perantx = np.median(datacolxxampdb_mask0,axis=1)
    medamp_peranty = np.median(datacolyyampdb_mask0,axis=1)
    ...
    lowpower_flags = set(flags[0]).difference(offline_flags)

    # use unflagged antennas to generate median spectrum
    flagmask = np.zeros((Nants,Nchans_perspw*Nspws))
    flagmask[flags[0],:] = 1
    ...
    medamp_allantsx = filters.median_filter(np.nanmedian(datacolxxampdb_mask.filled(),axis=0), size=25)
    medamp_allantsy = filters.median_filter(np.nanmedian(datacolyyampdb_mask.filled(),axis=0), size=25)

    stdarrayx = np.array( [np.nanmean(np.abs(filters.median_filter(antarr.filled(), size=25) - medamp_allantsx)) for antarr in datacolxxampdb_mask] )
    stdarrayy = np.array( [np.nanmean(np.abs(filters.median_filter(antarr.filled(), size=25) - medamp_allantsy)) for antarr in datacolyyampdb_mask] )
    ...
    flags2 = np.where( (stdarrayx > (np.nanmean(stdarrayx) + np.nanstd(stdarrayx))) | (stdarrayy > (np.nanmean(stdarrayy) + np.nanstd(stdarrayy))) )


    def write_flags_to_list(flaglist, flagfilename):
    ...
    write_flags_to_list(flagsall, 'flag_bad_ants.ants')
    write_flags_to_list(flags2[0], 'weird_spectrum.ants')
    write_flags_to_list(offline_flags, 'offline.ants')
    write_flags_to_list(lowpower_flags, 'low_power.ants')
    #flagsallstr = [str(flag) for flag in flagsall]
    #flagsallstr2 = ",".join(flagsallstr)
...
def corr_num_to_ant_num(antlist):
    configfile = '/lustre/mmanders/stageiii/phaseiii/sys-config.csv'
    config = np.genfromtxt(configfile, delimiter=',', skip_header=1, names=True, usecols=(1,31), dtype=['S7',int])
    inds = [i for i, e in enumerate(config['corr_num']) if e in set(antlist)]
    antnums = [int(config['antname'][i][4::]) for i in inds]
    return antnums
