...
time_step = 0
while time_step < 72:
    if f"41-82_{dirname}_{str(time_step).zfill(3)}.ms" in os.listdir(pathname):
        print(f"41-82_{dirname}_{str(time_step).zfill(3)}.ms exists. Skipping.")
        time_step += 1
    else:
        ...
        print(f"Concatenating files and saving to 41-82_{dirname}_{str(time_step).zfill(3)}.ms")
        virtualconcat(
            vis=datafile_list,
