...
for run_ind, run_path in enumerate(run_filepaths):
    ...
    plt.plot(k_edges_plot, power_plot, color=colors[run_ind], label=f"{run_names[run_ind]} residual")
    ...
    plt.plot(k_edges_plot, power_plot, color=colors[run_ind], label=f"{run_names[run_ind]} dirty")

    plt.xscale("log")
    plt.xlabel("k-perpendicular (h/Mpc)")
    plt.ylabel("Power")
plt.legend()
plt.savefig(f"{plot_save_dir}/kpar0_power.png")
