...
def cost_func_gaussian_fit(x, diff_fft_abs_squared, delay_array, bl_lengths, c=3e8):
    wedge_error = x[0]
    window_error_floor = x[1]
    boundary_slope = x[2] / c
    gaussian_peak = x[3]
    gaussian_width = x[4]
    fit = np.full_like(diff_fft_abs_squared, window_error_floor)
    fit += gaussian_peak * np.exp(
        -delay_array[np.newaxis, :] ** 2.0 / (2 * gaussian_width**2.0)
    )
    for delay_ind, delay_val in enumerate(delay_array):
        wedge_bls = np.where(
            bl_lengths * boundary_slope > np.abs(delay_val)
        )[0]
        if len(wedge_bls) > 0:
            fit[wedge_bls, delay_ind] = wedge_error
    return np.sum((diff_fft_abs_squared - fit) ** 2.0)
...
def run_gaussian_fit():
    ...
    x0 = np.array([7.060438044132621, 0.07, 0.6284790822752272, 4., 1e-6])
    step_fit_result = scipy.optimize.minimize(
        cost_func_gaussian_fit,
        x0,
        args=(
            diff_fft_abs.squeeze() ** 2.0,
            delay_array,
    ...
    print(f"Window Variance: {step_fit_result.x[1]}")
    print(f"Wedge Boundary Slope: {step_fit_result.x[2]}")
    print(f"Gaussian Peak: {step_fit_result.x[1]}")
    print(f"Gaussian Width: {step_fit_result.x[2]}")
